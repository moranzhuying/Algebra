...
\begin{example}{导子的标量扩张}{}
	沿用定义(\ref{def:epsilon-derivation})的记号.设$\bar{K}$是交换环,$\rho\in\Hom_{\mathsf{Ring}}\left(K,\bar{K}\right)$,$\bar{A},\bar{A}^{\prime},\bar{A}^{\prime\prime},\bar{B},\bar{B}^{\prime},\bar{B}^{\prime\prime}$分别是把$A,A^{\prime},A^{\prime\prime},B,B^{\prime},B^{\prime\prime}$通过$\rho$延拓到$\bar{K}$得到的分次$\bar{K}$-模,$\bar{\lambda}=\rho^{\ast}\left(\mu\right),\bar{\lambda}_{1}=\rho^{\ast}\left(\lambda\right),\bar{\lambda}_{2}=\rho^{\ast}\left(\lambda_{2}\right)$.若
	\begin{align*}
