...
\begin{definition}{可三角化自同态}{}
	沿用命题(\cref{prop:equivalent-condition-of-triangularisable-endomorphism-over-finite-dimensional-vector-spaces})的设定.其中的三个陈述之一为真时,称$\mathcal{T}$可三角化.
\end{definition}
...
\begin{remark}{}{}
	设$m\geq 1$,$\left(\left(\lambda_{i},k_{i}\right)\right)_{i=1}^{m}$是$K\times\N_{\geq 1}$中的序列,$\boldsymbol{A}\in\Mat_{n}\left(K\right)$且和$K$上的矩阵
	\begin{align*}
...
\begin{proposition}{矩阵的极小多项式的计算}{}
	设$\boldsymbol{A}\in\Mat_{n}\left(K\right),n\geq 1$.如果存在$m\geq 1$和$K\times\N_{\geq 1}$的序列$\left(\left(\lambda_{i},k_{i}\right)\right)_{i=1}^{m}$,使得
	\begin{align*}
...
\begin{corollary}{代数重数的几何含义}{}
	沿用命题(\cref{prop:general-eigensubspaces-decomposition-of-vector-spaces})的设定,则对每个$\lambda\in\mathscr{E}$,它作为$\Char_{\mathcal{T}}$的根的重数和$\gm\left(\lambda\right)$相等.
\end{corollary}
