...
\begin{definition}{辛群}{}
	沿用引理\ref{lem:well-definedness-of-symplectic-group}的记号.我们称$\Sp\left(\varphi\right)$是$\varphi$的辛群.
\end{definition}
...
\begin{definition}{辛方程}{}
	设$\boldsymbol{M}\in\mathbf{M}_{2m}\left(A\right)$.如果$\boldsymbol{M}^{\top}\boldsymbol{J}_{m}\boldsymbol{M}=\boldsymbol{J}_{m}$,则称$\boldsymbol{M}$满足$m$阶辛方程,公式$\boldsymbol{M}^{\top}\boldsymbol{J}_{m}\boldsymbol{M}=\boldsymbol{J}_{m}$称为$m$阶辛方程.
\end{definition}

\begin{proposition}{$m$阶辛矩阵的性质}{}
	设$\boldsymbol{M}\in\mathbf{M}_{2m}\left(A\right)$.
	\begin{enumerate}
