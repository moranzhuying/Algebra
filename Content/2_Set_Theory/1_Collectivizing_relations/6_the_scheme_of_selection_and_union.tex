...
\begin{definition}{}{}
	记号和假设同元定理(\cref{metathm:replacement-axiom}).$\left(\exists x\right)\left(\left(y=T\right)\wedge\left(x\in A\right)\right)$读作``对某个属于$A$的$x$,$y$可以表示为$T$的形式'',该公式构造的集合记作记作$\left\{T\left(x\right)\middle|x\in A\right\}$.
\end{definition}