...
记号约定:$K$是除环,$m,n\in\N,\boldsymbol{A}\coloneq\left(a_{i,j}\right)_{\substack{1\leq i\leq m\\1\leq j\leq n}}\in\mathbf{M}_{m,n}\left(K\right)$.
...
\begin{proposition}{矩阵的列秩不随除环扩充而改变}{}
	设$K_{0}$是$K$的子除环.若$\boldsymbol{A}\in\mathbf{M}_{m,n}\left(K_{0}\right)$,则$\dim_{K}\col\left(A\right)=\dim_{K_{0}}\col\left(\boldsymbol{A}\right)$.
\end{proposition}
...
\begin{proposition}{线性方程组有唯一解的增广矩阵判别法}{}
	设$\boldsymbol{B}\coloneq\left(b_{i}\right)_{1\leq i\leq n}$是$K$上的列矩阵,$\left(\boldsymbol{A}|\boldsymbol{B}\right)\in\mathbf{M}_{m,n+1}\left(K\right)$满足$\col_{i}\left(\left(\boldsymbol{A}|\boldsymbol{B}\right)\right)=
	\begin{cases}
