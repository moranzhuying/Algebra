...
\begin{proposition}{}{}
	记号同定义(\cref{def:evaluation-of-polynomial}),则有$A$-模的短正合列$0\xlongrightarrow{}\ker\left(\ev_{\mathbf{x}}\right)\xlongrightarrow{}A\left[\left(X_{i}\right)_{i\in I}\right]\xlongrightarrow{\ev_{\mathbf{x}}^{\circ}}A\left[\left(x_{i}\right)_{i\in I}\right]\xlongrightarrow{}0$.
\end{proposition}
...
\begin{definition}{多项式的偏次数}{}
	记号同命题(\cref{prop:degree-respect-to-a-subset-of-variables}).设$u\in A\left[\left(X_{i}\right)_{i\in I}\right]$非零.称$\deg\left(\phi^{-1}\left(u\right)\right)$为$u$关于不定元$\left(X_{k}\right)_{k\in K}$的次数(或偏次数).
\end{definition}
