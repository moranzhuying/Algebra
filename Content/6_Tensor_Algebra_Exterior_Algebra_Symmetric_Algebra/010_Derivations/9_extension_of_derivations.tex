...
\begin{example}{自同态诱导的代数导子}{}
	沿用命题(\cref{prop:extend-derivation-to-tensor-algebra-symmetric-algebra-and-exterior-algebra})的设定.取$d=0$,那么$d_{1}$是$A$-线性映射.
	\begin{itemize}
...
\begin{example}{外代数上的内插算子}{}
	沿用命题(\cref{prop:extend-derivation-to-tensor-algebra-symmetric-algebra-and-exterior-algebra})的设定,考虑$B=\bigwedge\left(M\right)$,$\Delta=\left\{0\right\}$的情形,并采用标准交换因子,则每个$x^{\ast}\in M^{\vee}$能唯一延拓为$\bigwedge\left(M\right)$上的$-1$次反导子$i\left(x^{\ast}\right)$.具体地,对$M$中每个序列$\left(x_{i}\right)_{i=1}^{n}$,有
	\begin{align*}
...
\begin{example}{对偶模和态射模上的导子}{}
	\begin{enumerate}
		\item 在命题(\cref{prop:derivation-over-multilinear-mapping-module})中,考虑$n=1$,$M_{1}=M,P=A$的情形.那么$M^{\vee}$上诱导了导子$d^{\ast}$,对每个$m\in M$和$m^{\ast}\in M^{\vee}$有
		\begin{align*}
