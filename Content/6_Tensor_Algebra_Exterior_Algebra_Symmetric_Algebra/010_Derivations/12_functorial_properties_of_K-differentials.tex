...
\begin{definition}{}{}
	沿用命题(\ref{prop:natuality-of-K-differential})的设定.我们把$v$记作$\Omega\left(u\right)$.
\end{definition}
...
\begin{definition}{}{}
	沿用命题(\ref{prop:natuality-of-K-differential})的设定.记$\Omega_{0}\left(u\right):u^{\ast}\left(\Omega_{K}\left(A\right)\right)\to\Omega_{K^{\prime}}\left(A^{\prime}\right)$是$u$诱导的$A^{\prime}$-线性映射.
\end{definition}

\begin{proposition}{基变换下的$K$-微分模同构}{}
	沿用命题(\ref{prop:natuality-of-K-differential})的设定.设$A^{\prime}=u^{\ast}\left(K^{\prime}\right)$,$\eta^{\prime}$和$u$是典范同态,则$\Omega_{0}\left(u\right)$是$A^{\prime}$-模同构.
\end{proposition}
